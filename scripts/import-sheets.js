...
function ratingToNumber(value) {
    ...
    if (rating === '⭐️' || rating === '⭐')
        return 1;
    if (rating === '⭐️⭐️' || rating === '⭐⭐')
        return 2;
    if (rating === '⭐️⭐️⭐️' || rating === '⭐⭐⭐')
        return 3;
    if (rating === '⭐️⭐️⭐️⭐️' || rating === '⭐⭐⭐⭐')
        return 4;
    if (rating === '⭐️⭐️⭐️⭐️⭐️' || rating === '⭐⭐⭐⭐⭐')
        return 5;
    return null;
}
...
async function getOrCreateSeries(name, genreId) {
...
async function getOrCreateBook(row) {
    ...
    const goodreadsUrl = clean(row['goodreads']) ||
        clean(row['Goodreads']) ||
...
async function ensureClubvisionCandidates(clubvisionId) {
...
function getChapterOrder(title) {
    const cleanTitle = clean(title);
    if (cleanTitle === 'Prólogo')
        return 0;
    if (cleanTitle === 'Epílogo')
        return 9998;
    if (cleanTitle.includes('Reflexión'))
        return 9999;
...
async function getOrCreateReadingForComment(bookTitle, chapterTitle) {
...
async function importReadingComments() {
    ...
    for (const row of rows) {
        ...
        if (!oldId || !bookTitle || !chapterTitle || !userName || !text)
            continue;
        const user = await prisma.user.findUnique({
            where: { name: userName },
        });
        ...
        await prisma.comment.upsert({
            ...
            update: {
                ...
                deletedAt: clean(row['Eliminado']).toUpperCase() === 'TRUE'
                    ? new Date()
                    : null,
            },
            create: {
...
//# sourceMappingURL=import-sheets.js.map