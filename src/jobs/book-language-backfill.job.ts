...
import { runMissingLanguageBackfillJob } from '../services/book-language-backfill.service.js';

type CommandDependencies = {
  run?: typeof runMissingLanguageBackfillJob;
...
export async function main(
  ...
  const run = dependencies.run ?? runMissingLanguageBackfillJob;
  ...
  try {
    ...
    errorOutput(JSON.stringify({
      ok: false,
      error: 'LANGUAGE_BACKFILL_FAILED',
      mensaje: 'El trabajo de idiomas no pudo completarse',
    }));
    return 1;
  } finally {
    await disconnect();
