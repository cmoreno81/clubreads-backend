...
export async function getLecturasActivas() {
    ...
    const lectorasByBook = new Map(readingBooks.map((item) => [item.bookId, item._count.userId]));
    ...
    for (const lectura of compartidas) {
        ...
        if (!book)
            continue;
        const yaExiste = resultado.some((item) => item.libro.trim().toLowerCase() === book.title.trim().toLowerCase());
        if (yaExiste)
            continue;
        resultado.push({
            libro: book.title,
    ...
    if (latestResult?.winnerTitle) {
        const yaExiste = resultado.some((item) => item.libro.trim().toLowerCase() ===
...
export async function crearLectura(data) {
    ...
    if (!title)
        return { ok: false, mensaje: 'Falta el libro' };
...
export async function getConfiguracionLectura(libro) {
...
export async function getComentariosLectura(libro, capitulo, usuarioActual) {
...
export async function enviarComentarioLectura(data) {
    ...
    if (!conversation)
        return { ok: false, mensaje: 'Capítulo no encontrado' };
...
export async function toggleLikeComentario(comentarioId, usuario) {
...
export async function editarComentarioLectura(comentarioId, comentario) {
...
export async function eliminarComentarioLectura(comentarioId) {
    if (!comentarioId) {
...
export async function editarRespuestaLectura(respuestaId, respuesta) {
    return editarComentarioLectura(respuestaId, respuesta);
}
export async function eliminarRespuestaLectura(respuestaId) {
    return eliminarComentarioLectura(respuestaId);
}
export async function getConversacionesLibro(libro) {
    const readings = await prisma.reading.findMany({
        where: {
            book: {
                title: libro.trim(),
