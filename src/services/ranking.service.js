...
export async function getRanking() {
    ...
    for (const item of library) {
        ...
        if (item.status === ReadingStatus.ABANDONED) {
            abandonados.set(item.book.title, (abandonados.get(item.book.title) ?? 0) + 1);
        }
    }
    for (const review of reviewsAbandonados) {
        abandonados.set(review.book.title, (abandonados.get(review.book.title) ?? 0) + 1);
    }
    ...
    const masDeseados = top(Array.from(deseados.entries())
        .map(([libro, total]) => ({ libro, total }))
        .sort((a, b) => b.total - a.total));
    const masLeidos = top(Array.from(leidos.entries())
        .map(([libro, total]) => ({ libro, total }))
        .sort((a, b) => b.total - a.total));
    const masAbandonados = top(Array.from(abandonados.entries())
        .map(([libro, total]) => ({ libro, total }))
        .sort((a, b) => b.total - a.total));
    const topLectoras = top(Array.from(lectoras.entries())
        .map(([usuario, total]) => ({ usuario, total }))
        .sort((a, b) => b.total - a.total));
    const mejorValorados = top(Array.from(valoraciones.entries())
        .map(([libro, data]) => ({
        libro,
