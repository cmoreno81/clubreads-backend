...
function getMood(valoracionMedia) {
    const media = Number(valoracionMedia);
    if (media >= 4.4)
        return 'El club está enamorado de las últimas lecturas 💜';
    if (media >= 4)
        return 'Las últimas lecturas están gustando mucho 📚';
    if (media >= 3.5)
        return 'Hay opiniones para todos los gustos 🤔';
    if (media > 0)
        return 'Necesitamos una lectura que nos reconcilie con el club 😅';
...
export async function getDashboard() {
    ...
    const contadorUsuarios = new Map();
    for (const item of finishedBooks) {
        ...
        if (itemMonth !== month)
            continue;
        contadorUsuarios.set(item.user.name, (contadorUsuarios.get(item.user.name) ?? 0) + 1);
    }
    const topUsuario = Array.from(contadorUsuarios.entries()).sort((a, b) => b[1] - a[1])[0];
    ...
    const leyendoAhoraResponse = Array.from(leyendoPorUsuario.entries()).map(([usuario, libros]) => ({
        usuario,
        libros,
        total: libros.length,
    }));
    ...
    let ultimaFechaLecturaActual = null;
    ...
    const topGenre = Array.from(genreCounter.entries()).sort((a, b) => b[1] - a[1])[0];
    ...
    return {
        resumen: {
            ...
            actividadMes: Array.from(contadorUsuarios.values()).reduce((sum, total) => sum + total, 0),
            valoracionMedia,
        },
