...
import { findImportedBookLanguage } from './book-cover.service.js';

const DEFAULT_BATCH_SIZE = 200;
const MAX_BATCH_SIZE = 500;
const LOOKUP_DELAY_MS = 250;
const LOOKUP_TIMEOUT_MS = 30_000;
const ADVISORY_LOCK_KEY = 1_835_101_730;

type BackfillDatabase = Pick<typeof prisma, 'book'>;
type LanguageLookup = typeof findImportedBookLanguage;
...
type JobDependencies = BackfillDependencies & {
  ...
  executeBackfill?: typeof backfillMissingBookLanguages;
};
  ...
  try {
    return await Promise.race([
      ...
      new Promise<never>((_resolve, reject) => {
        timer = setTimeout(() => reject(new Error('LANGUAGE_LOOKUP_TIMEOUT')), timeoutMs);
        timer.unref();
      }),
    ]);
  } finally {
...
export async function backfillMissingBookLanguages(
  options: { apply?: boolean; limit?: number } = {},
  dependencies: BackfillDependencies = {},
) {
  const startedAt = Date.now();
  const database = dependencies.database ?? prisma;
  const findLanguage = dependencies.findLanguage ?? findImportedBookLanguage;
  ...
  const matches: Array<{ bookId: string; title: string; language: string }> = [];
  let added = 0;
  let omitted = 0;
  let failures = 0;
  for (const book of books) {
    try {
      const language = await withTimeout(findLanguage({
        title: book.title,
      ...
      if (!language) {
        ...
        matches.push({ bookId: book.id, title: book.title, language });
        if (options.apply) {
...
export async function runMissingLanguageBackfillJob(
  options: { limit?: number } = {},
  dependencies: JobDependencies = {},
): Promise<LanguageBackfillSummary> {
  const backfillEnabled = process.env.LANGUAGE_BACKFILL_ENABLED ?? (
  ...
  if (
    ...
    throw new Error('El backfill de idiomas está desactivado');
  }
  const startedAt = Date.now();
  ...
  try {
    ...
    const execute = dependencies.executeBackfill ?? backfillMissingBookLanguages;
