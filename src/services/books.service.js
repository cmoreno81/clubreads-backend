...
function statusToFlutter(status) {
    ...
    if (status === ReadingStatus.ABANDONED)
        return 'ABANDONADO';
    if (status === ReadingStatus.REREADING)
        return 'RELEYENDO';
    return 'PENDIENTE';
}
function priorityToFlutter(priority) {
...
function ratingFromFlutter(value) {
    const rating = String(value ?? '').trim();
    if (!rating)
        return null;
    if (rating === '⭐' || rating === '⭐️')
        return 1;
    if (rating === '⭐⭐' || rating === '⭐️⭐️')
        return 2;
    if (rating === '⭐⭐⭐' || rating === '⭐️⭐️⭐️')
        return 3;
    if (rating === '⭐⭐⭐⭐' || rating === '⭐️⭐️⭐️⭐️')
        return 4;
    if (rating === '⭐⭐⭐⭐⭐' || rating === '⭐️⭐️⭐️⭐️⭐️')
        return 5;
    const numeric = Number(rating);
    return Number.isNaN(numeric) ? null : numeric;
}
function statusFromFlutter(estado) {
    if (estado === 'LEYENDO')
...
export async function getLibros(usuario) {
...
export async function getLibrosFinalizados() {
    ...
    return library.map((item) => {
        const review = item.book.reviews.find((review) => review.userId === item.userId);
        return {
            usuario: item.user.name,
            libro: item.book.title,
            genero: item.book.genre.name,
...
export async function anadirLibroExistente(usuario, libro) {
...
export async function iniciarLectura(usuario, libro) {
    return actualizarEstado(usuario, libro, 'LEYENDO');
}
export async function actualizarEstado(usuario, libro, estado, valoracion, reflexion) {
    const user = await prisma.user.findUnique({
        where: { name: usuario.trim() },
    });
    if (!user)
    ...
    const now = new Date();
    const statusDates = status === ReadingStatus.READING
...
export async function actualizarValoracion(usuario, libro, valoracion) {
    const user = await prisma.user.findUnique({
        where: { name: usuario.trim() },
    });
    if (!user)
...
function boolFromFlutter(value) {
...
export async function crearLibro(data) {
    ...
    if (!title)
        return { ok: false, mensaje: 'Falta el título del libro' };
    const user = await prisma.user.findUnique({
        where: { name: usuario },
    });
    if (!user)
...
function priorityFromFlutter(value) {
...
function ratingToFlutter(rating) {
    if (rating === 0)
        return '😞';
    if (!rating)
        return '';
