...
const POINTS_BY_POSITION = [12, 10, 8, 7, 6];
function getCurrentEdition() {
    const now = new Date();
    return `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
}
async function getCalculatedClubvisionStatus(clubvisionId) {
...
export async function getClubvision(usuario) {
...
export async function enviarVotacion(usuario, votos) {
...
export async function getMiVoto(usuario) {
    const user = await prisma.user.findUnique({
...
//# sourceMappingURL=clubvision.service.js.map